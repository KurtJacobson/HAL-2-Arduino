...
The serial link works as follows:
  ...
  LinuxCNC and the Arduino send each other single line of 4 numerical 'word'
  statements back and forth such as "codeNumber axis dataValue Checksum".
  The checksum is simply the sum of the preceeding arguments of the sentence.
  Sentences are deliniated with a semicolon.
  
  Some examples of sentences would be like the following:
  ...
  226  axis.N.joint-vel-cmd - (float, out) (units per minute)
...
// Use native USB port? (option for Arduino Due users ONLY)
// Does not work for virtual machines such as VirtualBox.
#define useDueNativePort false
