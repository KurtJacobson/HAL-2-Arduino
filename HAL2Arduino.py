...
def comSetup(ser, command, data1, data2):
    ...
    lastCommand="%s %s %s%s" % (str(command), str(data1), str(data2), ";") #, str(lastChecksum), ";")
        ...
        if linkMsg.find('ok') > -1:
            ...
            if comSetup(ser, 990, 0, 0) == firmware:
                ...
                print "comThread: Sending %s to %s" % (lastCommand, port)
                                ser.write(lastCommand)
                                for i in range(0,6): # attemp to send a command, but no more then 5 times.
                                    sleep(0.007) # Give the Arduino 7 millisec (per attempt) to respond.
                                    linkMsg = ser.readline()
...
def commandHandler(codesAccepted, axisesRequested):
    ...
    while threadsRun:
    for i in range(0,len(clientQueueArray)):
        clientQueue=clientQueueArray[i]
...
#                print "commandHandler: interpreted as %d %d %d %d" % (int(dataSet[0]), int(dataSet[1]), int(dataSet[2]), buildSum)
                if  dataSet[0] == dataSet[0]: #buildSum == int(dataSet[3]):
#               print "commandHandler: received %s" % msg
...
#                        print "commandHandler: Got %d %d %d %d" % (command, axis, val, buildSum)
                        print "commandHandler: Got %d %d %d" % (command, axis, val)
                        if command == 100:
                        ...
                        if command == 998:
                            timeVal=val*0.001
                            sleep(timeVal)
                            print "Sleeping: %dMs" % timeVal

                else:
#                    print "commandHandler: INVALID COMMAND, command:%d axis:%d val:%d sum:%d" % (dataSet[0], dataSet[1], dataSet[2], dataSet[3])
                    print "commandHandler: INVALID COMMAND, command:%d axis:%d val:%d" % (dataSet[0], dataSet[1], dataSet[2])
            except Exception:
